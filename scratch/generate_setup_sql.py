...
with open('scratch/old_export.json', 'r', encoding='utf-8') as f:
    data = json.load(f)

sql = """-- ============================================================
-- KNSDC — COMPLETE MASTER SETUP SCRIPT FOR NEW SUPABASE PROJECT
-- Target Project: https://fjscpohgysbelzkrkrxm.supabase.co
-- Generated automatically with all tables, realtime, RPC functions, and current data
-- ============================================================
...
# Add seeds
sql += "\n-- 18. INITIAL DATA SEEDING (FROM CURRENT SYSTEM)\n"

# sync_state
for s in data.get('sync', []):
    s_id = s['id'].replace("'", "''")
    s_payload = json.dumps(s['payload']).replace("'", "''")
    sql += f"INSERT INTO public.sync_state (id, payload) VALUES ('{s_id}', '{s_payload}'::jsonb) ON CONFLICT (id) DO UPDATE SET payload = EXCLUDED.payload;\n"

# staff_credentials
for sc in data.get('staff', []):
    em = sc['email'].replace("'", "''")
    ph = sc['password_hash'].replace("'", "''")
    nm = sc['name'].replace("'", "''")
    rl = sc.get('role', 'monitor').replace("'", "''")
    sql += f"INSERT INTO public.staff_credentials (email, password_hash, name, role) VALUES ('{em}', '{ph}', '{nm}', '{rl}') ON CONFLICT (email) DO UPDATE SET password_hash = EXCLUDED.password_hash, name = EXCLUDED.name, role = EXCLUDED.role;\n"

# events
for ev in data.get('events', []):
    ev_id = ev['id']
    ev_nm = (ev.get('name') or 'Event').replace("'", "''")
    sql += f"INSERT INTO public.events (id, name) VALUES ({ev_id}, '{ev_nm}') ON CONFLICT (id) DO NOTHING;\n"

with open('setup_new_supabase.sql', 'w', encoding='utf-8') as f:
    f.write(sql)

print('setup_new_supabase.sql created, size:', len(sql))
