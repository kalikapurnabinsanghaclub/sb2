      ...
      syncEngine.subscribe((state) => {
        ...
        if (state && state.participants) {
          state.participants.forEach(p => {
            if (p && p.present === undefined) p.present = true;
          });
        }
        ...
        activeEventIds.forEach(eid => {
          ...
          const queue = participants.filter(p => p.eventId == eid && p.stageStatus === 'queue').slice(0, 4);
          ...
          html += `
            <div class="event-live-section" style="margin-bottom: 40px; padding: 32px;">
              ...
              <div class="stage-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
                <!-- Insert Banner Here -->
                <div id="live-sponsor-banner-modal-${ev.id}" style="grid-column: 1 / -1; min-height: 80px; display:flex; align-items:center; justify-content:center;"></div>
                ...
                <!-- Queue Area - Hidden when empty, synced with Monitor UPCOMING (NEXT 4) -->
                ${queue.length > 0 ? `
      ...
      content.innerHTML = `
        <div style="background:#fff; width:100%; height:100%; border:15px double #1A237E; padding:30px; text-align:center; position:relative; box-sizing:border-box; display:flex; flex-direction:column; justify-content:center; align-items:center;">
          ...
          <div style="margin-bottom:15px;">
            <img src="logo.jpg" style="height:70px; width:auto; border-radius:50%; box-shadow:0 4px 10px rgba(0,0,0,0.1);" alt="KNS Logo">
          </div>

          <div style="font-family:'Cinzel Decorative', cursive; font-size:0.9rem; color:#1A237E; margin-bottom:5px;">Kalikapur Nabin Sangha</div>
          <div style="font-size:1.8rem; font-weight:800; color:#1A237E; margin-bottom:15px; font-family:'Playfair Display', serif; text-transform:uppercase; letter-spacing:1px;">Certificate of Participation</div>
          
          <div style="margin:10px 0; font-size:1rem; color:#555;">This is to certify that</div>
          <div style="font-size:2.2rem; font-weight:900; color:#FF6B35; font-family:'Playfair Display', serif; border-bottom:2px solid #eee; display:inline-block; padding:0 30px; margin-bottom:15px;">${p.name}</div>
          
          <div style="margin:10px 0; font-size:1rem; color:#333; line-height:1.5;">
            ...
            <strong style="color:#1A237E;">${event.name}</strong> <br>
            held on <strong>${event.disp}</strong> at <strong>Kalikapur, Kolkata</strong>.
          </div>

          <div style="margin-top:30px; width:100%; display:flex; justify-content:space-between; align-items:flex-end; padding:0 40px;">
            <div style="text-align:center;">
              <div style="border-top:1px solid #333; width:120px; margin-top:30px; font-size:0.75rem;">Secretary</div>
            </div>
            <div style="text-align:center;">
              <div style="font-size:0.65rem; color:#777; margin-bottom:5px;">Certificate No: ${certNo}</div>
              <div style="background:#1A237E; color:#fff; padding:4px 12px; font-size:0.75rem; font-weight:800; border-radius:4px; letter-spacing:1px;">KNSDC OFFICIAL</div>
            </div>
            <div style="text-align:center;">
              <div style="border-top:1px solid #333; width:120px; margin-top:30px; font-size:0.75rem;">President</div>
            </div>
          </div>
    ...
    function printCert() {
      ...
      win.document.write('<html><head><title>KNSDC Certificate</title>');
      win.document.write('<style>@import url("https://fonts.googleapis.com/css2?family=Playfair+Display:wght@700;900&family=Cinzel+Decorative:wght@700&display=swap"); body{margin:0; padding:20px;}</style>');
    ...
    function downloadData(type) {
      ...
      if (type === 'results-pdf') {
        const results = window.LIVE_RESULTS || [];
        ...
        const grouped = {};
        results.forEach(r => {
          const rnd = r.round || 'audition';
          if (!grouped[rnd]) grouped[rnd] = [];
          grouped[rnd].push({ Rank: r.rank, ID: r.id || 'N/A', Name: r.name, Category: r.cat, Score: r.score });
        });
        generatePDFRoundwise(eventName, orgName, 'Full Results — All Rounds', publishDate, publishTime, roundOrder, roundLabels, grouped);
        ...
        const cats = [...new Set(results.map(r => r.cat))].sort();
        const pages = cats.map(cat => ({
          cat,
          rows: results
            .filter(r => r.cat === cat)
            .sort((a, b) => b.score - a.score)
            .map((r, i) => ({ Rank: i + 1, ID: r.id || 'N/A', Name: r.name, Score: r.score }))
        }));
        if (pages.length === 0) { showToast("No Results", "No results available yet.", "⚠️"); return; }
        generatePDFCategoryPages(eventName, orgName, 'Category-wise Sheet', publishDate, publishTime, pages);

      } else if (type === 'promotion') {
        const promoted = window.PROMOTED || [];
        ...
        const grouped = {};
        promoted.forEach(p => {
          const rnd = p.round || 'qualified';
          if (!grouped[rnd]) grouped[rnd] = [];
          const fullP = state.participants.find(x => x.name === p.name);
          grouped[rnd].push({ ID: fullP ? fullP.id : 'N/A', Name: p.name, Category: p.cat });
        });
    ...
    function generatePDFReport(eventName, orgName, title, date, time, headers, rows) {
      ...
      setTimeout(() => window.print(), 600);<\/script>
      </body></html>`);
      win.document.close();
    }
    ...
    // Full Results PDF — grouped by round
    function generatePDFRoundwise(eventName, orgName, title, date, time, roundOrder, roundLabels, grouped) {
      const win = window.open('', '_blank', 'height=800,width=900');
      ...
      roundOrder.forEach(rnd => {
        ...
        body += `<div class="round-section">
          ...
          <table><thead><tr><th>#</th><th>ID</th><th>Name</th><th>Category</th><th>Score</th></tr></thead>
          <tbody>${list.map((p, i) => `<tr><td>${i + 1}</td><td>${p.ID}</td><td>${p.Name}</td><td>${p.Category}</td><td>${p.Score}</td></tr>`).join('')}</tbody></table>
        </div>`;
      });
    ...
    // Category-wise Sheet — one page per category
    function generatePDFCategoryPages(eventName, orgName, title, date, time, pages) {
      const win = window.open('', '_blank', 'height=800,width=900');
      let body = pages.map((pg, pi) => `
        <div class="cat-page">
          ...
          <div class="cat-header">
            <div>
              <div class="cat-name">📂 ${pg.cat}</div>
              <div class="cat-count">${pg.rows.length} participant${pg.rows.length > 1 ? 's' : ''}</div>
            </div>
          </div>
          <table><thead><tr><th>Rank</th><th>ID</th><th>Name</th><th>Score</th></tr></thead>
          <tbody>${pg.rows.map(r => `<tr><td>${r.Rank}</td><td>${r.ID}</td><td>${r.Name}</td><td>${r.Score}</td></tr>`).join('')}</tbody></table>
      ...
      roundOrder.forEach(rnd => {
        ...
        win.document.write(`
          <div class="round-section">
            ...
            <tbody>${list.map((p, i) => `<tr><td>${i + 1}</td><td>${p.ID}</td><td>${p.Name}</td><td>${p.Category}</td></tr>`).join('')}</tbody></table>
          </div>`);
      });
      if (!hasData) {
    ...
    function searchParticipant() {
      ...
      // Get scoring subjects and compute category-wise max and average
      const subjects = window._KNSDC_SUBJECTS || ev?.subjects || state.subjects || [];
      const numJudges = window._KNSDC_NUM_JUDGES || 1;
      const maxTotalScore = window._KNSDC_MAX_SCORE || (numJudges * subjects.reduce((a, s) => a + (s.maxMarks || 10), 0));

      // Compute category average (average score of all participants in same category)
      ...
      if (subjects.length > 0) {
        ...
        const getJudgeScores = (j) => {
          const agr = judgeAgreements.find(a => 
            ...
            (a.name && j.name && String(j.name).toLowerCase() === String(j.name).toLowerCase())
          );
          return (p.scores && p.scores[j.id]) || (agr && p.scores && p.scores[agr.id]) || null;
        };

        const scoredJudges = judges.filter(j => getJudgeScores(j) !== null);
        ...
        if (scoredJudges.length === 0) {
          subjectBreakdownHTML = `
            <div style="margin-top:16px; background:#f8fafc; border-radius:14px; padding:16px; border:1px solid #e2e8f0;">
              <div style="font-size:0.7rem; color:#7C3AED; font-weight:800; text-transform:uppercase; letter-spacing:1px; margin-bottom:10px;">📊 Judge-wise Category Breakdown</div>
              <div style="font-size:0.8rem; color:#64748b; font-style:italic;">Scores will appear here once submitted by the judges.</div>
            </div>`;
        } else {
          subjectBreakdownHTML = `
            <div style="margin-top:16px; background:#f8fafc; border-radius:14px; padding:16px; border:1px solid #e2e8f0;">
              ...
              ${scoredJudges.map(judge => {
                const jScores = getJudgeScores(judge) || {};
                let judgeTotal = 0;
                let judgeMax = 0;
                
                subjects.forEach(s => {
                  judgeTotal += (typeof jScores === 'object') ? (Number(jScores[s.id]) || 0) : 0;
                  judgeMax += (s.maxMarks || 10);
                });

                let judgeHtml = `
                  <div style="margin-bottom:12px; padding:12px; background:#fff; border-radius:10px; border:1px solid #e2e8f0;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px; border-bottom:1px solid #e2e8f0; padding-bottom:4px;">
                      <span style="font-size:0.8rem; font-weight:800; color:#334155;">🧑‍⚖️ ${judge.name}</span>
                      <span style="font-size:0.75rem; font-weight:800; color:#7C3AED; background:rgba(124,58,237,0.1); padding:2px 8px; border-radius:10px;">${judgeTotal} / ${judgeMax} pts</span>
                    </div>`;
                
                subjects.forEach(s => {
                  let subScore = (typeof jScores === 'object') ? (Number(jScores[s.id]) || 0) : 0;
                  const subMax = s.maxMarks || 10;
                  ...
                  judgeHtml += `
                    <div style="margin-bottom:8px;">
                      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px;">
                        <span style="font-size:0.75rem; font-weight:600; color:#475569;">${s.name || 'Subject'}</span>
      ...
      area.innerHTML = `
        <div style="padding:24px; border-radius:20px; border:2px solid rgba(124,58,237,0.2); text-align:left; animation:bounceIn 0.5s ease both; background:linear-gradient(135deg, #f5f3ff 0%, #ede9fe 100%); box-shadow: 0 8px 24px rgba(124,58,237,0.08);">
          ...
          <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:10px; margin-bottom:16px;">
            <div style="background:#ffffff; padding:14px; border-radius:14px; text-align:center; border:1px solid rgba(124,58,237,0.1);">
              <div style="font-size:0.65rem; color:#7C3AED; font-weight:800; text-transform:uppercase; letter-spacing:0.5px;">Your Score</div>
              <div style="font-size:1.4rem; color:#1E293B; font-weight:900; margin-top:4px;">${totalScore}</div>
            </div>
            <div style="background:#ffffff; padding:14px; border-radius:14px; text-align:center; border:1px solid rgba(234,88,12,0.1);">
              <div style="font-size:0.65rem; color:#EA580C; font-weight:800; text-transform:uppercase; letter-spacing:0.5px;">Max Score</div>
              <div style="font-size:1.4rem; color:#1E293B; font-weight:900; margin-top:4px;">${maxTotalScore}</div>
            </div>
            <div style="background:#ffffff; padding:14px; border-radius:14px; text-align:center; border:1px solid rgba(245,158,11,0.1);">
              <div style="font-size:0.65rem; color:#D97706; font-weight:800; text-transform:uppercase; letter-spacing:0.5px;">Cat. Avg</div>
              <div style="font-size:1.4rem; color:#1E293B; font-weight:900; margin-top:4px;">${catAvg}</div>
            </div>
          </div>
    ...
    function init() {
      ...
      if (role === 'public') {
        ...
        const portalMap = {
          ...
          'umpire': 'KNSDC-Judge.html',
          'referee': 'KNSDC-Judge.html'
        };
    ...
    function renderSponsorBanners() {
      const cont = document.getElementById('global-sponsors-container');
      if (!cont) return;
      
      const allSponsors = EVENTS.flatMap(e => (e.switchStates && e.switchStates.sponsors) ? e.switchStates.sponsors : []);
      ...
      cont.innerHTML = allSponsors.map(sp => `
        <a ${sp.link ? `href="${sp.link}" target="_blank"` : ''} style="text-decoration:none; display:flex; align-items:center; gap:12px; background:rgba(255,255,255,0.9); border:1px solid rgba(0,0,0,0.05); padding:10px 16px; border-radius:16px; box-shadow:0 4px 15px rgba(0,0,0,0.05); transition:transform 0.2s;" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'">
          <img src="${sp.logo}" alt="Sponsor" style="height:36px; width:auto; object-fit:contain; border-radius:6px;"/>
          <div style="display:flex; flex-direction:column; justify-content:center;">
            <div style="font-size:0.8rem; font-weight:900; color:#1e293b; text-transform:uppercase; letter-spacing:0.5px;">${sp.name}</div>
            ${sp.tagline ? `<div style="font-size:0.65rem; color:#64748b; font-weight:700;">${sp.tagline}</div>` : ''}
          </div>
        </a>
      `).join('');
    }
    
    function startSponsorAnimation(containerId, sponsors) {
      const container = document.getElementById(containerId);
      if (!container || !sponsors || sponsors.length === 0) return;
      
      // Use a hash to prevent unnecessary re-renders when syncEngine fires
      const sponsorsHash = sponsors.map(s => s.name).join('|');
      if (container.dataset.sponsorsHash === sponsorsHash) return;
      container.dataset.sponsorsHash = sponsorsHash;
      
      // Build the HTML for each sponsor in the strip
      const sponsorItems = sponsors.map(sp => `
        <div style="display: inline-flex; align-items: center; gap: 15px; margin: 0 40px; vertical-align: middle;">
          ${sp.logo ? `<img src="${sp.logo}" style="height: 45px; object-fit: contain; filter: drop-shadow(0 0 5px rgba(255,255,255,0.2)); border-radius: 4px;" />` : ''}
          <div style="display: inline-flex; flex-direction: column; justify-content: center; text-align: left;">
            <span style="color:#fff; font-weight:900; font-size:1.1rem; letter-spacing:1px; font-family:'Playfair Display',serif;">${sp.name}</span>
            ${sp.tagline ? `<span style="color:#FFD23F; font-size:0.75rem; font-weight:800; text-transform:uppercase; letter-spacing:0.5px;">${sp.tagline}</span>` : ''}
          </div>
        </div>
      `).join('');
      
      // Duplicate 4 times to ensure seamless infinite scrolling regardless of screen width
      const content = sponsorItems.repeat(4);
      
      // Inject CSS for the marquee if not already present
      if (!document.getElementById('live-sponsor-marquee-style')) {
        const style = document.createElement('style');
        style.id = 'live-sponsor-marquee-style';
        style.textContent = `
          @keyframes scroll-sponsor-strip {
          ...
          .sponsor-marquee-content {
            ...
            animation: scroll-sponsor-strip ${Math.max(15, sponsors.length * 8)}s linear infinite;
          }
          .sponsor-marquee-wrapper:hover .sponsor-marquee-content {
      ...
      container.innerHTML = `
        <div class="sponsor-marquee-wrapper">
    ...
    function renderEvents() {
      ...
      EVENTS.forEach((ev, i) => {
        const div = document.createElement("div");
        div.className = "ch glass-pink";
        div.style.cssText = `border-radius:20px;padding:28px 20px;text-align:center;border-top:4px solid ${ev.col};animation:bounceIn 0.6s ${i * 0.1}s ease both; cursor:pointer;`;
        div.onclick = (e) => {
        ...
        const btnText = isRegOpen ? "Register Now" : "Registration Closed";
    ...
    function checkLiveStatus() {
      ...
      if (secDot && secText && secBadge) {
        if (isOffline) {
          ...
          secBadge.style.background = 'linear-gradient(135deg, #334155, #475569)';
        } else {
          secDot.style.background = '#fff';
          secDot.style.boxShadow = '0 0 10px #fff';
          secDot.style.animation = 'pulseDot 1.5s infinite';
          secText.textContent = 'LIVE NOW';
          secBadge.style.background = 'linear-gradient(135deg, #1A237E, #3b82f6)';
        }
      }
      ...
      if (liveSection) {
        liveSection.style.display = hasLiveStage ? 'block' : 'none';
      }
      ...
      const cardOuter = document.getElementById('live-card-outer');
      ...
      if (liveEvent) {
        ...
        if (cardOuter) cardOuter.style.display = 'block';
        ...
        if (futureEvents.length > 0) {
          renderUpcomingLiveUI(futureEvents[0]); // Ensure this renders to fallbackContainer or its own logic
        } else {
          fallbackContainer.innerHTML = `
    ...
    function renderLiveUI(ev) {
      ...
      if (headerContainer) {
        headerContainer.innerHTML = `
          <div style="background: linear-gradient(135deg, #7C3AED, #6366F1, #4F46E5); padding: 35px 40px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 20px; position: relative; overflow: hidden;">
            ...
            <div style="position: relative; z-index: 1;">
              <div style="display: inline-flex; align-items: center; gap: 8px; background: rgba(255, 255, 255, 0.15); color: #fff; padding: 6px 18px; border-radius: 30px; margin-bottom: 12px; font-size: 0.82rem; font-weight: 800; border: 1px solid rgba(255, 255, 255, 0.25); letter-spacing: 1px;">
                <span class="live-dot" style="background: #FFD23F; box-shadow: 0 0 10px #FFD23F;"></span> ${ev.cat.toUpperCase()} LIVE
              </div>
              <h3 style="font-family: 'Playfair Display', serif; font-size: 2.4rem; color: #fff; font-weight: 900; letter-spacing: -1px; line-height: 1.1;">${ev.name}</h3>
              <p id="liveEventSchedule" style="color: rgba(255, 255, 255, 0.85); margin-top: 8px; font-size: 0.95rem; font-weight: 500;">📅 ${effectiveDisp}&nbsp;&nbsp;•&nbsp;&nbsp;📍 ${effectiveVenue}</p>
            </div>
            <div style="display:flex; align-items:center; gap:16px; position: relative; z-index: 1;">
              <div style="background: rgba(255, 255, 255, 0.18); backdrop-filter: blur(10px); border-radius: 18px; padding: 14px 24px; border: 1px solid rgba(255, 255, 255, 0.25); text-align: center;">
                <div style="color: rgba(255, 255, 255, 0.85); font-size: 0.72rem; margin-bottom: 4px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px;">Status</div>
                <div style="color: #86EFAC; font-weight: 900; font-size: 1.05rem; display: flex; align-items: center; gap: 6px;">
                  <span style="width: 8px; height: 8px; background: #86EFAC; border-radius: 50%; display: inline-block; box-shadow: 0 0 8px rgba(134,239,172,0.5);"></span> Active
                </div>
              </div>
      ...
      const bodyContainer = document.getElementById("live-card-body");
      if (bodyContainer) {
        bodyContainer.innerHTML = `
          <div style="padding: 40px;">
            <div style="background: linear-gradient(135deg, #f5f3ff 0%, #ede9fe 50%, #e0e7ff 100%); border-radius: 24px; padding: 50px 30px; text-align: center; margin-bottom: 40px; position: relative; overflow: hidden; border: 1px solid rgba(139, 92, 246, 0.15); box-shadow: 0 8px 32px rgba(139, 92, 246, 0.06);">
              ...
              <div style="position: relative; z-index: 1;">
                <div style="font-size: 3.2rem; margin-bottom: 12px; filter: drop-shadow(0 5px 15px rgba(124,58,237,0.15));">🎭</div>
                <h4 style="color: #7C3AED; font-family: 'Playfair Display', serif; font-size: 1.1rem; margin-bottom: 10px; font-weight: 800; letter-spacing: 3px; text-transform: uppercase;">On Stage Now</h4>
                <div class="performer-card-live" style="background: transparent; border: none; padding: 0;">
                  <div class="performer-name-live" id="currentPerformerName" style="font-size: 2.8rem; font-weight: 900; color: #1E293B; text-shadow: none;">${onStageName}</div>
                  <div class="performer-category-live" id="currentPerformerCat" style="font-size: 1.1rem; color: #64748B; font-weight: 600; margin-top: 5px;">${onStageCat}</div>
                </div>
              </div>
            </div>
            ...
            <div class="participant-hub-grid" style="display:grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap:25px; margin-bottom:40px;">
              ...
              <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; flex-wrap:wrap; gap:15px; background:#f8fafc; padding:15px; border-radius:16px; border:1px solid #e2e8f0;">
                ...
                <div style="display:flex; gap:10px; flex:1; justify-content:flex-end; min-width:280px;">
                  <div style="position:relative; flex:1; max-width:250px;">
                    ...
                    <input type="text" id="promoSearch" placeholder="Search name or ID..." oninput="window.renderPromotion()" style="width:100%; padding:8px 10px 8px 32px; border-radius:10px; border:1px solid #e2e8f0; background:#fff; color:#1E293B; font-size:0.85rem; outline:none; transition:border-color 0.3s;" onfocus="this.style.borderColor='#7C3AED'" onblur="this.style.borderColor='#e2e8f0'">
                  </div>
      ...
      if(ev.switchStates && ev.switchStates.sponsors && ev.switchStates.sponsors.length > 0) {
        startSponsorAnimation(`live-global-sponsor-banner`, ev.switchStates.sponsors);
      }
    }


    function renderUpcomingLiveUI(ev) {
      const container = document.getElementById("liveContentFallback");
      if (!container) return;
      container.innerHTML = `
    ...
    function renderQueue() {
      ...
      const liveQueue = participants.filter(p => p.eventId == activeEventId && p.stageStatus === 'queue');
    ...
    function renderResults() {
      ...
      const eventJudges = (rsState.judges || []).filter(j => {
        if (j.present === false) return false;
      ...
      const numJudges = eventJudges.length > 0 ? eventJudges.length : ((rsState.judges || []).filter(j => j.present !== false).length || 1);
    ...
    function renderPromotion() {
      ...
      if (PROMOTED.length > 0) {
        const targetRound = PROMOTED[0].round;
        const titleEl = document.getElementById("promotionTitle");
        const subEl = document.getElementById("promotionSubtitle");
        if (titleEl) titleEl.innerHTML = `🏆 Promoted to ${targetRound.charAt(0).toUpperCase() + targetRound.slice(1)} Round`;
        if (subEl) subEl.innerHTML = `Participants who successfully qualified for the ${targetRound} stage.`;
      }
      ...
      if (filtered.length === 0 && search) {
        list.innerHTML = `<div style="grid-column:1/-1; padding:40px; text-align:center; color:#94A3B8;">No participants found matching "${search}"</div>`;
        return;
      }
      ...
      filtered.forEach((p, i) => {
        ...
        const div = document.createElement("div");
        div.className = "ch";
        div.style.cssText = `border-radius:18px; padding:16px 20px; border:1px solid rgba(124,58,237,0.12); display:flex; align-items:center; gap:14px; animation:bounceIn 0.4s ${i * 0.1}s ease both; background:#ffffff; box-shadow: 0 2px 12px rgba(124,58,237,0.05);`;
        const pData = { id: fullP ? fullP.id : 'N/A', phone: fullP ? (fullP.phone || '') : '', name: p.name, cat: p.cat, catId: fullP ? fullP.catId : '', eventId: fullP ? fullP.eventId : '', rank: 'Qualified', score: 'N/A' };
        const pEncoded = btoa(unescape(encodeURIComponent(JSON.stringify(pData))));
        ...
        div.innerHTML = `
          ...
          <div style="flex:1">
            <div style="font-weight:800; font-size:1.05rem; color:#1E293B; letter-spacing:0.3px;">${p.name}</div>
            <div style="font-size:0.82rem; color:#7C3AED; font-weight:800; text-transform:uppercase; letter-spacing:1px;">${p.cat}</div>
          </div>
          <div style="display:flex; align-items:center; gap:12px;">
            <div style="background:rgba(16,185,129,0.08); color:#059669; padding:4px 10px; border-radius:30px; font-size:0.75rem; font-weight:900; border:1px solid rgba(16,185,129,0.15);">QUALIFIED</div>
            <button class="btn btn-p" style="padding:8px 12px; font-size:1.1rem; border-radius:10px; background:#f5f3ff; border:1px solid rgba(124,58,237,0.15); color:#7C3AED;" onclick="window.generateParticipantCertificate(JSON.parse(decodeURIComponent(escape(atob('${pEncoded}')))))" title="Download Certificate">🎓</button>
          </div>
        `;
    ...
    function renderRoles() {
      ...
      if (!div) return;
      div.innerHTML = "";
      const roles = ["public", "admin", "monitor", "host", "judge", "umpire", "referee"];
      roles.forEach(role => {
      ...
      fields.forEach((f, idx) => {
        ...
        if (f.type === 'text') {
          ...
          html += `<div class="fg" style="margin-bottom:20px;"><label style="${labelStyle}">${f.label}${f.required?' *':''}</label>
            <div style="display:flex;flex-direction:column;gap:10px;">
              <input type="text" placeholder="Paste Link here" style="${inputStyle}" onchange="document.getElementById('${id}').value = this.value">
              <div style="font-size:0.8rem;color:#64748b;text-align:center;font-weight:bold;margin:4px 0;">— OR UPLOAD FILE —</div>
          ...
          html += `
            <div class="fg" style="margin-bottom:20px;">
              ...
              <div style="padding:24px; border:2px dashed #4f46e5; border-radius:16px; background:#f5f3ff; text-align:center; box-shadow: 0 4px 15px rgba(79,70,229,0.05);">
                ...
                <button type="button" class="btn btn-p" style="padding:10px 20px; font-size:0.9rem; font-weight:700; background:#4f46e5; border-radius:10px; color:#fff; cursor:pointer;" onclick="showToast('Payment', 'Offline payment option is also active at the venue. Online gateway integration demo is ready!', 'ℹ️')">💳 Pay Now (Demo)</button>
              </div>
            </div>
          `;
        }
    ...
    window.uploadPublicFileToCloud = async function(input, targetId) {
      ...
      const url = await syncEngine.uploadFile(file);
    ...
    function showEventDetail(id) {
      ...
      // Banner
      const bannerEl = document.getElementById('edm-banner');
      if (ev.banner) {
      ...
      document.getElementById('edm-cat').textContent = ev.cat || 'Event';
      document.getElementById('edm-title').textContent = ev.name || 'Unknown Event';
      document.getElementById('edm-date').textContent = ev.disp || '--';
      document.getElementById('edm-venue').textContent = ev.venue || '--';
      document.getElementById('edm-org').textContent = ev.org || 'KNSDC';
      document.getElementById('edm-desc').textContent = ev.description || 'No description provided.';
      
      const schedCont = document.getElementById('edm-schedule-container');
      const schedBody = document.getElementById('edm-schedule');
      ...
      if (ev.roundSchedules && Object.keys(ev.roundSchedules).length > 0) {
        ...
        rounds.forEach(r => {
          ...
          if (s && (s.date || s.venue)) {
            scheduleHtml += `<div style="margin-bottom:8px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(0,0,0,0.05); padding-bottom:8px;">
              <div>
                <strong style="text-transform:capitalize; color:#4f46e5;">${r}</strong>
                <div style="font-size:0.85rem; color:#64748b;">${s.venue || 'TBA'}</div>
              </div>
              <div style="text-align:right;">
                <div style="font-weight:700; color:#1e293b; font-size:0.9rem;">${s.date || 'TBA'}</div>
                <div style="font-size:0.8rem; color:#64748b;">${s.time || 'TBA'}</div>
              </div>
            </div>`;
          }
        });
      ...
      const regBtn = document.getElementById('edm-reg-btn');
      if (isRegOpen) {
      ...
      document.getElementById('edm-support-btn').onclick = () => {
        closeEventDetailModal();
    ...
    function handleSignup() {
      ...
      Object.entries(formData).forEach(([label, val]) => {
        ...
        else if (lbl.includes('AGE')) age = parseInt(val) || 0;
      });
      ...
      const newP = {
        ...
        stageStatus: "waiting", present: true,
