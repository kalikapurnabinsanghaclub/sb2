...
class LocalSync {
  ...
  setData(updater) {
    ...
    if (this.state.currentOnStage !== oldState.currentOnStage) {
      broadcastPayload.currentOnStage = this.state.currentOnStage;
      shouldBroadcast = true;
    }

    if (JSON.stringify(this.state.onstageTimer) !== JSON.stringify(oldState.onstageTimer)) {
      broadcastPayload.onstageTimer = this.state.onstageTimer;
      shouldBroadcast = true;
    }

    if (this.state.hostScoresVisible !== oldState.hostScoresVisible) {
      broadcastPayload.hostScoresVisible = this.state.hostScoresVisible;
      shouldBroadcast = true;
    }

    if (JSON.stringify(this.state.judges) !== JSON.stringify(oldState.judges)) {
      broadcastPayload.judges = this.state.judges;
      shouldBroadcast = true;
    }

    // Check participants for updates (scores, stageStatus, present, round)
    const oldPartsMap = new Map((oldState.participants || []).map(p => [String(p.id), p]));
    const changedParts = [];
    (this.state.participants || []).forEach(p => {
      const oldP = oldPartsMap.get(String(p.id));
      if (!oldP || 
    ...
    const oldStateToCompare = { ...oldState };
    delete oldStateToCompare.lastUpdated;

    const hasRealChanges = JSON.stringify(stateToCompare) !== JSON.stringify(oldStateToCompare);
