...
class LocalSync {
  ...
  async loadEvents() {
    try {
      ...
      const finalGlobalCats = (globalCats && globalCats.length > 0) ? globalCats : mappedEvents.flatMap(e => e.categories || []);
      const finalGlobalVens = (globalVens && globalVens.length > 0) ? globalVens : mappedEvents.flatMap(e => e.venues || []);
      const finalGlobalSubs = (subData && subData.length > 0) ? subData.map(s => ({
        id: Number(s.id) || s.id,
        name: s.name,
        maxMarks: Number(s.max_marks) || 10,
        desc: s.description || '',
        eventId: s.event_id
      })) : mappedEvents.flatMap(e => e.subjects || []);
