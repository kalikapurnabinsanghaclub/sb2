...
class LocalSync {
        ...
        .on(
          ...
          (payload) => {
            ...
            if (payload.new && payload.new.payload) {
              ...
              // Smart participant merge: keep local scores if they are newer than what came from cloud.
              // This prevents a race condition where another portal's sync overwrites locally-saved
              // judge scores that haven't propagated to Supabase yet.
              ...
              if (mergedParticipants && this.state.participants && this.state.participants.length > 0) {
                ...
                mergedParticipants = mergedParticipants.map(np => {
                  ...
                  if (!lp) return np;
                  // If local participant has scores that cloud doesn't (or cloud scores are empty), keep local
                  ...
                  np.image = lp.image;
                  np.posterData = lp.posterData;
                  np.qrDataUrl = lp.qrDataUrl;
  ...
  async updateEvent(id, eventObj) {
    try {
      // ── Helper: try updating a single column, silently skip if column missing ──
      ...
      // ── STEP 1: GUARANTEED CORE — columns confirmed in supabase-schema.sql ──
      // Schema: id, title, date, time, venue, description, category, image, form_fields, "publicReg", "stagePreview", "resultPublic"
      const safeObj = {};
      if (eventObj.name        !== undefined) safeObj.title       = eventObj.name;
      if (eventObj.startDate   !== undefined) safeObj.date        = eventObj.startDate;
      if (eventObj.startTime   !== undefined) safeObj.time        = eventObj.startTime;
      if (eventObj.venue       !== undefined) safeObj.venue       = eventObj.venue;
      if (eventObj.description !== undefined) safeObj.description = eventObj.description;
      if (eventObj.banner      !== undefined) safeObj.image       = eventObj.banner;
      if (eventObj.type        !== undefined) safeObj.category    = eventObj.type;
      if (eventObj.formFields  !== undefined) {
        safeObj.form_fields = typeof eventObj.formFields === 'string'
          ? eventObj.formFields
          : JSON.stringify(eventObj.formFields);
      }

      // Update publicReg / stagePreview / resultPublic from switchStates if present
      if (eventObj.switchStates !== undefined) {
        const sw = eventObj.switchStates;
        if (sw.publicReg    !== undefined) safeObj['"publicReg"']    = sw.publicReg;
        if (sw.stagePreview !== undefined) safeObj['"stagePreview"'] = sw.stagePreview;
        if (sw.resultPublic !== undefined) safeObj['"resultPublic"'] = sw.resultPublic;
      }

      const { error: safeError } = await this.supabase.from('events').update(safeObj).eq('id', id);
      if (safeError) {
        console.error('[LocalSync] Core update failed:', safeError);
        return { success: false, message: safeError.message || safeError.details || 'Database write error' };
      }

      // ── STEP 2: Try extended columns that may have been added via migration ──
      // These are silently skipped if the column doesn't exist yet
      if (eventObj.org      !== undefined) {
        // Try 'organizer' first, then 'org'
      ...
      if (eventObj.endDate      !== undefined) await tryCol({ end_date:        eventObj.endDate || null });
      if (eventObj.endTime      !== undefined) await tryCol({ end_time:        eventObj.endTime || null });
      if (eventObj.capacity     !== undefined) await tryCol({ capacity:        eventObj.capacity || null });
      if (eventObj.switchStates !== undefined) await tryCol({ switch_states:   eventObj.switchStates });
      if (eventObj.roundSchedules !== undefined) {
        await tryCol({
          round_schedules: typeof eventObj.roundSchedules === 'string'
            ? eventObj.roundSchedules
            : JSON.stringify(eventObj.roundSchedules)
        });
      }

      // ── STEP 3: Update local in-memory state ──
      this.setData(state => {
        const newFormFields = { ...(state.eventFormFields || {}) };
        if (eventObj.formFields) {
  ...
  async updateParticipant(id, pObj) {
    try {
      ...
      // Force-save sync_state immediately when scores are involved (bypass 1s debounce)
      // This guarantees scores are persisted even if public_registrations update fails
      if (pObj.scores !== undefined || pObj.roundScores !== undefined || pObj.roundComments !== undefined) {
        this.forceSaveStateToSupabase().catch(err =>
          console.warn('[LocalSync] Force-save after score update failed:', err)
        );
      }
