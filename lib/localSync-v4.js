...
class LocalSync {
  ...
  async signIn(email, password) {
    ...
    // ── Step 1: Check Supabase judge_credentials table (hashed) ──
    if (this.supabase) {
      try {
        const passwordHash = await sha256(password);
        ...
        if (!judgeErr && judgeRow) {
          ...
          localStorage.setItem('kns_user', JSON.stringify({ name: judgeRow.name, email: emailNorm, agreement_id: judgeRow.agreement_id }));
          console.log('[LocalSync] Judge authenticated via Supabase judge_credentials:', emailNorm);
          return {
            success: true,
            role: 'judge',
            user: { email: emailNorm, user_metadata: { fullname: judgeRow.name, role: 'judge' } }
          };
        }
      } catch (e) {
        console.warn('[LocalSync] judge_credentials check failed, continuing:', e.message);
      }
    }

    // ── Step 2: Fallback — check local judgeAgreements (plain text, offline mode) ──
    ...
    if (matchedLocal) {
      localStorage.setItem('kns_role', 'judge');
      localStorage.setItem('kns_user', JSON.stringify({ name: matchedLocal.name, email: emailNorm, agreement_id: matchedLocal.agreement_id }));
      console.log('[LocalSync] Judge authenticated via local judgeAgreements (offline fallback):', emailNorm);
      return {
        success: true,
        role: 'judge',
    ...
    // ── Step 3: Try Supabase Auth (for Admin / Monitor / Host accounts) ──
    ...
    try {
      ...
      // ── Look up role from user_roles table (more reliable than metadata) ──
      let role = user.user_metadata?.role || null;
      let fullname = user.user_metadata?.fullname || null;
      try {
        ...
        if (roleRow) {
          role = roleRow.role;
          fullname = roleRow.fullname || fullname;
          console.log('[LocalSync] Role loaded from user_roles table:', role);
        }
      } catch (e) {
        console.warn('[LocalSync] Could not fetch user_roles, using metadata fallback:', e.message);
      }
