...
class LocalSync {
  ...
  async updateEvent(id, eventObj) {
    try {
      // ── Helper: try a single-column update, return false if column missing ──
      const tryCol = async (colObj) => {
        const { error } = await this.supabase.from('events').update(colObj).eq('id', id);
        if (error) {
          console.warn('[LocalSync] Column update skipped:', Object.keys(colObj).join(','), '|', error.message);
          return false;
        }
        return true;
      };

      // ── STEP 1: Absolute-safe core — these columns definitely exist ──
      const safeObj = {};
      ...
      if (eventObj.active    !== undefined) safeObj.active      = eventObj.active;

      const { error: safeError } = await this.supabase.from('events').update(safeObj).eq('id', id);
      if (safeError) {
        console.error('[LocalSync] Safe-core update failed:', safeError);
        return { success: false, message: safeError.message || safeError.details || 'Database write error' };
      }

      // ── STEP 2: Try ambiguous columns individually (column name may vary by DB version) ──

      // Event type: try 'category' first (schema SQL name), then 'type'
      if (eventObj.type !== undefined) {
        const ok = await tryCol({ category: eventObj.type });
        if (!ok) await tryCol({ type: eventObj.type });
      }

      // Organizer: try 'organizer' first, then 'org'
      ...
      if (eventObj.formFields !== undefined) {
        const ff = typeof eventObj.formFields === 'string'
          ? eventObj.formFields
          : JSON.stringify(eventObj.formFields);
        await tryCol({ form_fields: ff });
      }

      // ── STEP 3: Extended columns (added by migration SQL — soft fail) ──
      ...
      // ── STEP 4: Update local in-memory state ──
      this.setData(state => {
        const newFormFields = { ...(state.eventFormFields || {}) };
        if (eventObj.formFields) {
