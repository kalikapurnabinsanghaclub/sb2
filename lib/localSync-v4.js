...
class LocalSync {
  ...
  handleBroadcastUpdate(payload) {
    ...
    if (payload.upcomingEvents && Array.isArray(payload.upcomingEvents)) {
      // Update events with fresh formFields from Monitor broadcast
      const events = [...(this.state.events || [])];
      payload.upcomingEvents.forEach(broadcastEv => {
        const idx = events.findIndex(e => String(e.id) === String(broadcastEv.id));
        if (idx !== -1) {
          // Merge formFields from broadcast into existing event
          if (broadcastEv.formFields && broadcastEv.formFields.length > 0) {
            events[idx] = { ...events[idx], formFields: broadcastEv.formFields };
            changed = true;
          }
        }
      });
      if (changed) this.state.events = events;
      // Also update upcomingEvents list
      this.state.upcomingEvents = payload.upcomingEvents;
      changed = true;
    }

    if (payload.eventFormFields && typeof payload.eventFormFields === 'object') {
      // Merge eventFormFields map so all clients have fresh form data
  ...
  setData(updater) {
    ...
    // Broadcast eventFormFields when changed — so Monitor's form saves reach ALL devices instantly
    if (JSON.stringify(this.state.eventFormFields) !== JSON.stringify(lastSaved.eventFormFields)) {
      broadcastPayload.eventFormFields = this.state.eventFormFields;
      broadcastPayload.upcomingEvents = (this.state.events || []).map(ev => ({
        id: ev.id,
        title: ev.name,
        ...
        formFields: (this.state.eventFormFields && this.state.eventFormFields[ev.id] && this.state.eventFormFields[ev.id].length > 0)
          ? this.state.eventFormFields[ev.id]
          : (ev.formFields || []),
