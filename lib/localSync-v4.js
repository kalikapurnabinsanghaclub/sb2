...
class LocalSync {
  ...
  async createParticipant(pObj) {
    try {
      ...
      const { error } = await supabase.from('public_registrations').insert([dbObj]);
  ...
  async updateParticipant(id, pObj) {
    try {
      ...
      const { error } = await supabase.from('public_registrations').update(dbObj).eq('id', id);
  ...
  async deleteParticipant(id) {
    try {
      const { error } = await supabase.from('public_registrations').delete().eq('id', id);
  ...
  async createCategory(cObj) {
    try {
      ...
      await supabase.from('categories').insert([dbObj]);
  ...
  async updateCategory(id, cObj) {
    try {
      ...
      await supabase.from('categories').update(dbObj).eq('id', id);
      this.setData(state => ({
        ...state,
  ...
  async createVenue(vObj) {
    try {
      ...
      await supabase.from('venues').insert([dbObj]);
  ...
  async updateVenue(id, vObj) {
    try {
      ...
      await supabase.from('venues').update(dbObj).eq('id', id);
      this.setData(state => ({
        ...state,
  ...
  async createAgreement(aObj) {
    try {
      ...
      const { error } = await supabase.from('judge_agreements').insert([dbObj]);
  ...
  async updateAgreement(id, aObj) {
    try {
      ...
      const { error } = await supabase.from('judge_agreements').update(dbObj).eq('id', id);
  ...
  async deleteAgreement(id) {
    try {
      const { error } = await supabase.from('judge_agreements').delete().eq('id', id);
  ...
  async createSubject(sObj) {
    try {
      ...
      const { error } = await supabase.from('scoring_subjects').insert([dbObj]);
      if (error) console.error('[LocalSync] Error creating subject', error);
      this.setData(state => ({ ...state, subjects: [...(state.subjects || []), sObj] }));
      return true;
    } catch(e) { console.error(e); return false; }
  }
  ...
  async updateSubject(id, sObj) {
    try {
      ...
      const { error } = await supabase.from('scoring_subjects').update(dbObj).eq('id', id);
      if (error) console.error('[LocalSync] Error updating subject', error);
      this.setData(state => ({
        ...state,
        subjects: (state.subjects || []).map(s => String(s.id) === String(id) ? { ...s, ...sObj } : s)
      }));
      return true;
    } catch(e) { console.error(e); return false; }
  }
  ...
  async deleteSubject(id) {
    try {
      const { error } = await supabase.from('scoring_subjects').delete().eq('id', id);
      if (error) console.error('[LocalSync] Error deleting subject', error);
      this.setData(state => ({
        ...state,
        subjects: (state.subjects || []).filter(s => String(s.id) !== String(id))
      }));
      return true;
    } catch(e) { console.error(e); return false; }
  }
  ...
  async submitContactMessage(msgObj) {
    try {
      const { error } = await supabase.from('public_messages').insert([msgObj]);
      if (error) {
  ...
  async submitRating(ratingVal) {
    try {
      const { error } = await supabase.from('public_ratings').insert([{ rating: ratingVal }]);
      if (error) {
