...
class LocalSync {
  ...
  async stagePush(participantId, venueId = null) {
    ...
    // 3. Background DB Persistence (non-blocking, 0 UI delay)
    if (this.supabase) {
      (async () => {
        try {
          if (targetPart) {
            await this.supabase.from('public_registrations').update({
              stage_status: 'on-stage',
              present: true
            }).eq('id', targetPart.id);
          }
          ...
          console.warn('[stagePush] DB update registration error:', dbErr);
        }
      })();
    }

    if (this.forceSaveStateToSupabase) {
      this.forceSaveStateToSupabase().catch(e => console.warn('[stagePush] force-save error:', e));
    }
    return true;
  }
  ...
  async stageDrop(participantId = null, venueId = null) {
    ...
    if (this.supabase && droppedParts.length > 0) {
      (async () => {
        try {
          ...
          console.warn('[stageDrop] DB update error:', dbErr);
        }
      })();
    }

    if (this.forceSaveStateToSupabase) {
      this.forceSaveStateToSupabase().catch(e => console.warn('[stageDrop] force-save error:', e));
    }
    return true;
  }
  ...
  async stageComplete(participantId = null, venueId = null) {
    ...
    // 3. Background DB Persistence (non-blocking)
    if (this.supabase && completedParts.length > 0) {
      (async () => {
        try {
          ...
          console.warn('[stageComplete] DB update error:', dbErr);
        }
      })();
    }

    if (this.forceSaveStateToSupabase) {
      this.forceSaveStateToSupabase().catch(e => console.warn('[stageComplete] force-save error:', e));
    }
    return true;
  }
