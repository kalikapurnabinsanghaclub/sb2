...
class LocalSync {
  ...
  async uploadFile(file, pathPrefix = '') {
    ...
    // 1. Try uploading to Cloudflare R2 (Zero Egress, Free Unlimited Bandwidth)
    try {
    ...
    try {
      ...
      const mappedEvents = data.map(dbEv => {
        ...
        if (dbEv.scoring_subjects || dbEv.subjects) {
          try {
            directSubs = typeof (dbEv.scoring_subjects || dbEv.subjects) === 'string' ? JSON.parse(dbEv.scoring_subjects || dbEv.subjects) : (dbEv.scoring_subjects || dbEv.subjects);
          } catch(e) {}
        }
        ...
        return {
          ...
          subjects: (eventSubjects && eventSubjects.length > 0) ? eventSubjects : (Array.isArray(directSubs) ? directSubs : []),
      ...
      const finalGlobalSubs = (subData && subData.length > 0)
        ? subData.map(s => ({
            id: Number(s.id) || s.id,
  ...
  async createSubject(sObj) {
    try {
      ...
      let { error } = await this.supabase.from('scoring_subjects').insert([fullObj]);
      if (error) {
        console.warn('[LocalSync] Subject full insert notice, using legacy columns:', error.message);
        const legacyObj = {
          id: sObj.id,
          subject_name: sObj.name,
          max_score: sObj.maxMarks,
          event_id: sObj.eventId
        };
        await this.supabase.from('scoring_subjects').insert([legacyObj]);
      }
      ...
      if (sObj.eventId) {
        try {
          const curState = this.getData() || {};
          const curEv = (curState.events || []).find(e => String(e.id) === String(sObj.eventId));
          if (curEv) {
            const updatedSubs = [...((curEv.subjects || []).filter(s => String(s.id) !== String(sObj.id))), sObj];
            await this.supabase.from('events').update({ scoring_subjects: JSON.stringify(updatedSubs) }).eq('id', sObj.eventId);
          }
        } catch(evErr) {}
      }
  ...
  async updateSubject(id, sObj) {
    try {
      ...
      let { error } = await this.supabase.from('scoring_subjects').update(fullObj).eq('id', id);
      if (error) {
        const legacyObj = {
          subject_name: sObj.name,
          max_score: sObj.maxMarks
        };
        await this.supabase.from('scoring_subjects').update(legacyObj).eq('id', id);
      }

      // Also sync to parent event in events table
      const curState = this.getData() || {};
      const existingSub = (curState.subjects || []).find(s => String(s.id) === String(id));
      const evId = sObj.eventId || (existingSub && existingSub.eventId);
      if (evId) {
        try {
          const curEv = (curState.events || []).find(e => String(e.id) === String(evId));
          if (curEv && curEv.subjects) {
            const updatedSubs = curEv.subjects.map(s => String(s.id) === String(id) ? { ...s, ...sObj } : s);
            await this.supabase.from('events').update({ scoring_subjects: JSON.stringify(updatedSubs) }).eq('id', evId);
          }
        } catch(evErr) {}
      }
    ...
    try {
      ...
      if (evId) {
        try {
          const curEv = (curState.events || []).find(e => String(e.id) === String(evId));
          if (curEv && curEv.subjects) {
            const remainingSubs = curEv.subjects.filter(s => String(s.id) !== String(id));
            await this.supabase.from('events').update({ scoring_subjects: JSON.stringify(remainingSubs) }).eq('id', evId);
          }
        } catch(evErr) {}
      }
