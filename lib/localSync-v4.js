...
class LocalSync {
  ...
  async setJudgePresence(identifier, isPresent = true, eventId = null, venueId = null) {
    ...
    this.setData(state => {
      ...
      let target = judgesList.find(j => 
        (j.email && String(j.email).toLowerCase().trim() === idNorm) || 
        (j.name && String(j.name).toLowerCase().trim() === idNorm) ||
        (j.id && String(j.id) === idNorm)
      );
      ...
      if (target) {
        ...
        const agr = allAgreements.find(a => 
          (a.email && String(a.email).toLowerCase().trim() === idNorm) || 
          (a.name && String(a.name).toLowerCase().trim() === idNorm) ||
          (a.id && String(a.id) === idNorm)
        );
