    ...
    try {
      ...
      if (evId) {
        try {
          ...
          const curEv = (curState.events || []).find(e => String(e.id) === String(evId));
          if (curEv) {
            const updatedSubs = [...((curEv.subjects || []).filter(s => String(s.id) !== String(numId))), normalizedSub];
      ...
      if (evId) {
        try {
          ...
          if (curEv && curEv.subjects) {
            const updatedSubs = curEv.subjects.map(s => String(s.id) === String(id) ? { ...s, ...normalizedSub } : s);
            await this.supabase.from('events').update({ scoring_subjects: JSON.stringify(updatedSubs) }).eq('id', evId);
          }
        } catch(evErr) {}
      }
