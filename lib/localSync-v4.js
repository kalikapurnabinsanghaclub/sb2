...
class LocalSync {
    ...
    this.setData(state => {
      ...
      if (idx !== -1) {
        ...
        const updated = {
          ...
          email: dbP.email !== undefined ? dbP.email : existing.email,
          eventId: dbP.event_id !== undefined ? dbP.event_id : existing.eventId,
          ...
          status: dbP.status !== undefined ? dbP.status : existing.status,
          formAnswers: dbP.form_data !== undefined ? (dbP.form_data || {}) : existing.formAnswers,
          formLocked: dbP.form_data ? (dbP.form_data._formLocked === true) : existing.formLocked,
          isReady: (dbP.form_data && dbP.form_data._isReady !== undefined) ? dbP.form_data._isReady : existing.isReady,
  ...
  insertParticipantFromDb(dbP) {
    if (!dbP || !dbP.id || dbP.status === 'Archived') return;
    const pidStr = String(dbP.id);
    this.setData(state => {
      const parts = [...(state.participants || [])];
  ...
  mapSingleDbParticipant(p) {
    return {
      ...
      formLocked: p.form_data ? (p.form_data._formLocked === true) : false,
      isReady: p.form_data && p.form_data._isReady !== undefined ? p.form_data._isReady : false
    };
  }
  ...
  // ─── Supabase Storage, MongoDB Atlas & Base64 Image Upload ───────────────────────────
  async uploadFile(file, pathPrefix = '') {
    if (!file) return null;

    // 1. Try uploading to Cloudflare R2 via backend API (Zero Egress, Free Unlimited Bandwidth)
    try {
      ...
      console.warn('[LocalSync] Cloudflare R2 upload endpoint error, attempting fallback:', r2Err);
    }

    // 2. Fallback: Try uploading to Supabase Storage if Supabase is connected
    if (this.supabase && this.supabase.storage) {
      try {
        const bucketName = 'knsdc-registration';
        const ext = file.name ? file.name.split('.').pop() : 'png';
        const cleanPrefix = pathPrefix ? pathPrefix.replace(/^\/+|\/+$/g, '') : 'banners';
    ...
    // 2. Try uploading to MongoDB Atlas via backend API if available
    try {
      const formData = new FormData();
      ...
      // MongoDB API route not reachable, continue to Base64
    }

    // 3. Base64 Fallback: Reads file as Data URL string
    return new Promise((resolve) => {
      const reader = new FileReader();
      reader.onload = function(e) {
        const base64Url = e.target.result;
        console.log('[LocalSync] Image converted to Base64 data URL.');
        resolve(base64Url);
      };
  ...
  async loadEvents() {
    try {
      const { data, error } = await this.supabase.from('events').select('id, name, date, start_date, end_date, venue, location, organizer, status, entry_fee, form_fields, is_active, created_at');
      if (error) {
        console.error('[LocalSync] Error loading events from Supabase', error);
        return this.state.events || [];
      }

      // Load categories, venues, judge agreements, and scoring subjects from database tables with explicit column lists
      let catData = [];
      let venData = [];
      let agrData = [];
      let subData = [];
      try {
        const { data: cData } = await this.supabase.from('categories').select('id, name, color, age_min, age_max, event_id');
        if (cData) catData = cData;
      } catch(e) { console.error('Error loading categories:', e); }

      try {
        const { data: vData } = await this.supabase.from('venues').select('id, name, location, capacity, dates, event_id');
        if (vData) venData = vData;
      } catch(e) { console.error('Error loading venues:', e); }

      try {
        const { data: sData } = await this.supabase.from('scoring_subjects').select('id, name, max_marks, description, event_id');
        if (sData) subData = sData;
      } catch(e) { console.error('Error loading subjects:', e); }

      try {
        const { data: aData } = await this.supabase.from('judge_agreements').select('id, name, phone, email, password, city, event_id, date, date_upto, time, venue_id, venue_name, spec, amount, advance, notes, status, submitted, payment_received, photo_url, agreed_tc, signature');
        if (aData) {
          agrData = aData.map(a => ({
            id: Number(a.id) || a.id,
            name: a.name,
  ...
  async updateEvent(id, eventObj) {
    try {
      ...
      if (eventObj.roundSchedules !== undefined || eventObj.otherPrizes !== undefined || eventObj.whatsapp !== undefined || eventObj.facebook !== undefined || eventObj.allowDonations !== undefined || eventObj.targetGoal !== undefined || eventObj.upiId !== undefined) {
  ...
  async stagePush(participantId, venueId = null) {
    ...
    this.setData(state => {
      ...
      // 1. Move any performer currently on stage to 'done' (same venue or globally)
      ...
      // 2. Set target performer to 'on-stage'
    ...
    if (this.forceSaveStateToSupabase) {
      await this.forceSaveStateToSupabase().catch(e => console.warn('[stagePush] force-save error:', e));
    }
    return true;
  }
  ...
  async stageDrop(participantId = null, venueId = null) {
    ...
    if (this.forceSaveStateToSupabase) {
      await this.forceSaveStateToSupabase().catch(e => console.warn('[stageDrop] force-save error:', e));
    }
    return true;
  }
  ...
  async stageComplete(participantId = null, venueId = null) {
    ...
    if (this.forceSaveStateToSupabase) {
      await this.forceSaveStateToSupabase().catch(e => console.warn('[stageComplete] force-save error:', e));
    }
    return true;
  }
  ...
  async loadParticipants() {
    ...
    try {
      ...
      const { data, error } = await this.supabase
        .from('public_registrations')
        .select('id, name, phone, age, gender, email, event_id, category, venue, form_data, scores, round_scores, round_comments, comment, status, created_at')
        .neq('status', 'Archived');
      if (error) {
