...
class LocalSync {
  ...
  async updateEvent(id, eventObj) {
    try {
      const dbObj = {};
      if (eventObj.name !== undefined)        dbObj.title         = eventObj.name;
      if (eventObj.org !== undefined)         dbObj.organizer     = eventObj.org;
      if (eventObj.type !== undefined)        dbObj.category      = eventObj.type;
      if (eventObj.venue !== undefined)       dbObj.venue         = eventObj.venue;
      if (eventObj.startDate !== undefined)   dbObj.date          = eventObj.startDate;
      if (eventObj.startTime !== undefined)   dbObj.time          = eventObj.startTime;
      if (eventObj.endDate !== undefined)     dbObj.end_date      = eventObj.endDate   || null;
      if (eventObj.endTime !== undefined)     dbObj.end_time      = eventObj.endTime   || null;
      if (eventObj.capacity !== undefined)    dbObj.capacity      = eventObj.capacity  || null;
      if (eventObj.banner !== undefined)      dbObj.image         = eventObj.banner;
      if (eventObj.description !== undefined) dbObj.description   = eventObj.description;
      if (eventObj.active !== undefined)      dbObj.active        = eventObj.active;
      if (eventObj.roundSchedules !== undefined) {
        dbObj.round_schedules = typeof eventObj.roundSchedules === 'string'
          ? eventObj.roundSchedules
          : JSON.stringify(eventObj.roundSchedules);
      }
      if (eventObj.formFields !== undefined) {
        dbObj.form_fields = typeof eventObj.formFields === 'string'
          ? eventObj.formFields
