...
class LocalSync {
  ...
  async updateEvent(id, eventObj) {
    try {
      ...
      if (eventObj.startDate   !== undefined) await tryCol({ date: eventObj.startDate });
      if (eventObj.startTime   !== undefined) await tryCol({ time: eventObj.startTime });
      ...
      if (eventObj.banner      !== undefined) await tryCol({ image: eventObj.banner });
