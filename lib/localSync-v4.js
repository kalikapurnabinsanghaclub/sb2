            ...
            if (payload.new && payload.new.payload) {
              ...
              if (mergedParticipants && this.state.participants && this.state.participants.length > 0) {
                ...
                mergedParticipants = mergedParticipants.map(np => {
                  ...
                  // 1. BUG-FIX: Split Optimistic Mutation Lock
                  //    Stage/queue/presence fields: 3s (need fast cross-portal sync)
                  //    Score fields: 15s (need long protection on slow networks)
                  //    Previously ALL fields used 15s, causing "ghost" stage states
                  //    to persist for 15s and block correct server corrections.
                  const hasStageChange = lp.stageStatus !== undefined || lp.queueOrder !== undefined || lp.present !== undefined || lp.roundPresence !== undefined;
                  const lockWindow = hasStageChange ? 3000 : 15000;
                  const isLocallyLocked = lp.localMutatedAt && (now - lp.localMutatedAt < lockWindow);
                  if (isLocallyLocked) {
                    return {
                      ...np,
