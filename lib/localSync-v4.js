...
class LocalSync {
    ...
    try {
      ...
      if (data && data.payload) {
        ...
        if (!this.state.lastUpdated || fetchedState.lastUpdated > this.state.lastUpdated) {
          // Preserve switch data, host assignments, and judge agreements loaded from their tables
          ...
          const preservedAgreements = this.state.agreements;
          const preservedJudgeAgreements = this.state.judgeAgreements;
          this.state = { ...this.state, ...fetchedState };
          // Re-apply preserved switch data, host assignments, and judge agreements
          ...
          if (preservedAgreements) this.state.agreements = preservedAgreements;
          if (preservedJudgeAgreements) this.state.judgeAgreements = preservedJudgeAgreements;
        ...
        // Also merge localStorage switch data (survives race conditions)
        try {
          const lsEventSwitches = localStorage.getItem('knsdc_eventSwitches');
          if (lsEventSwitches) {
            const parsed = JSON.parse(lsEventSwitches);
            this.state.eventSwitches = { ...(this.state.eventSwitches || {}), ...parsed };
          }
          const lsSwitchStates = localStorage.getItem('knsdc_switchStates');
          if (lsSwitchStates) {
            this.state.switchStates = { ...(this.state.switchStates || {}), ...JSON.parse(lsSwitchStates) };
          }
          const lsSystemStatus = localStorage.getItem('knsdc_systemStatus');
          if (lsSystemStatus) {
          ...
          (payload) => {
            ...
            if (payload.new && payload.new.payload) {
              ...
              if (!this.state.lastUpdated || newState.lastUpdated > this.state.lastUpdated) {
                // Preserve host assignments and judge agreements
                const preservedHostAssignments = this.state.hostAssignments;
                const preservedAgreements = this.state.agreements;
                const preservedJudgeAgreements = this.state.judgeAgreements;
  ...
  async loadEvents() {
    try {
      ...
      // Load categories, venues, and judge agreements from database tables
      let catData = [];
      let venData = [];
      let agrData = [];
      try {
  ...
  async loadParticipants() {
    ...
    try {
      ...
      const mappedParts = data.map(p => ({
        ...
        email: p.email || '',
        eventId: p.event_id,
        catId: Number(p.cat_id) || null,
        venueId: Number(p.venue_id) || null,
        date: p.reg_date,
