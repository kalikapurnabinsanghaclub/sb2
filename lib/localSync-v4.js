...
function mergeHostAssignments(listA, listB = null) {
  if (Array.isArray(listB)) {
    return listB.filter(h => h && h.email && h.email.trim().toLowerCase() !== 'x@' && h.email.trim().toLowerCase() !== 'x');
  }
  const merged = {};
  (listA || []).forEach(h => {
    if (h && h.email) {
      const em = h.email.trim().toLowerCase();
      if (em !== 'x@' && em !== 'x') {
        merged[em] = h;
      }
    }
...
class LocalSync {
  ...
  init() {
    ...
    try {
      ...
      if (lsHostAssignments) {
        try {
          const parsed = JSON.parse(lsHostAssignments);
          this.state.hostAssignments = (Array.isArray(parsed) ? parsed : []).filter(h => h && h.email && h.email.trim().toLowerCase() !== 'x@' && h.email.trim().toLowerCase() !== 'x');
        } catch(e) { this.state.hostAssignments = []; }
    ...
    // ── Real-time Staff Authentication & Automatic Lockout Heartbeat ──
    this.startStaffHeartbeat();
  ...
  async runAudioGarbageCollector() {
    ...
    for (const p of this.state.participants) {
      ...
      Object.entries(p.formAnswers).forEach(([k, v]) => {
         if (typeof v === 'string' && (
           v.includes('/storage/v1/object/public/knsdc-registration/') ||
           v.includes('workers.dev') ||
           v.includes('r2.dev') ||
           /\.(mp3|wav|ogg|m4a|aac)($|\?)/i.test(v) ||
           /audio|music|sound|track/i.test(k)
         )) {
            // Extract the upload timestamp from the filename if it matches our Date.now() format
            const tsMatch = v.match(/\/(\d{13})_/) || v.match(/(\d{13})/);
      ...
      if (needsUpdate) {
          // If we are deleting audio, we can also clean up the completion timestamp
          delete updatedFormAnswers._audioCompletedAt;
          
          for (const fileUrl of filesToDelete) {
             console.log(`[AutoClean] Deleting audio file: ${fileUrl}`);
             try {
               await this.deleteFile(fileUrl);
             } catch (e) {
               console.error(`[AutoClean] Error deleting ${fileUrl}:`, e);
             }
          }
          
          // Update the DB and local state so the URL is gone forever
          const updatePayload = { formAnswers: updatedFormAnswers };
          if (p.musicUrl && filesToDelete.includes(p.musicUrl)) {
            updatePayload.musicUrl = '';
          }
          await this.updateParticipant(p.id, updatePayload);
      }
    }
  }
    ...
    try {
      ...
      if (data && data.payload) {
        ...
        if (fetchedState.hostAssignments) {
          this.state.hostAssignments = (fetchedState.hostAssignments || []).filter(h => h && h.email && h.email.trim().toLowerCase() !== 'x@' && h.email.trim().toLowerCase() !== 'x');
        }
    ...
    this.setData(state => {
      ...
      if (idx !== -1) {
        ...
        const updated = {
          ...
          venueId: dbP.venue !== undefined ? (isNaN(Number(dbP.venue)) ? dbP.venue : (Number(dbP.venue) || existing.venueId)) : ((formAns && (formAns['Venue Field'] || formAns['Venue'] || formAns['venue'])) || existing.venueId || null),
          venue: dbP.venue || (formAns && (formAns['Venue Field'] || formAns['Venue'] || formAns['venue'])) || existing.venue || '',
  ...
  mapSingleDbParticipant(p) {
    ...
    return {
      ...
      venueId: p.venue ? (isNaN(Number(p.venue)) ? p.venue : (Number(p.venue) || p.venue)) : ((formAns && (formAns['Venue Field'] || formAns['Venue'] || formAns['venue'])) || null),
      venue: p.venue || (formAns && (formAns['Venue Field'] || formAns['Venue'] || formAns['venue'])) || '',
  ...
  async broadcastFormUpdate(eventFormFields) {
    ...
    try {
      const evData = (this.state.events || []).map(ev => ({
        ...
        publicRatingPos: (ev.switchStates || {}).publicRatingPos !== undefined ? Number((ev.switchStates || {}).publicRatingPos) : ((ev.switchStates || {}).publicVoting ? 3 : 0)
      }));
  ...
  async deleteStaffMember(email) {
    ...
    try {
      ...
      // Direct table delete fallback on staff_credentials
      if (!rpcSuccess) {
      ...
      // Also directly delete from judge_credentials and judge_agreements in Supabase
      try {
        await this.supabase.from('judge_credentials').delete().eq('email', emailLower);
        await this.supabase.from('judge_agreements').delete().eq('email', emailLower);
      } catch (jErr) {
        console.warn('[LocalSync] Direct judge tables cleanup notice:', jErr);
      }

      // Comprehensive removal from global state sync (staff, hosts, directors, partners, judges, events)
      try {
        let globalState = this.state || {};
        
        if (globalState.staff) {
          globalState.staff = globalState.staff.filter(s => (s.email || '').trim().toLowerCase() !== emailLower);
        }
        if (globalState.hostAssignments) {
          globalState.hostAssignments = globalState.hostAssignments.filter(h => (h.email || '').trim().toLowerCase() !== emailLower);
        }
        if (globalState.directorAssignments) {
          globalState.directorAssignments = globalState.directorAssignments.filter(d => (d.email || '').trim().toLowerCase() !== emailLower);
        }
        if (globalState.partnerAssignments) {
          globalState.partnerAssignments = globalState.partnerAssignments.filter(p => (p.email || '').trim().toLowerCase() !== emailLower);
        }
        if (globalState.judgeAgreements) {
          globalState.judgeAgreements = globalState.judgeAgreements.filter(a => (a.email || '').trim().toLowerCase() !== emailLower);
        }
        if (Array.isArray(globalState.events)) {
          globalState.events.forEach(e => {
            if (Array.isArray(e.staff)) {
              e.staff = e.staff.filter(s => (s || '').trim().toLowerCase() !== emailLower);
            }
          });
        }
        ...
        try {
          if (typeof localStorage !== 'undefined') {
            if (globalState.hostAssignments) {
              localStorage.setItem('knsdc_hostAssignments', JSON.stringify(globalState.hostAssignments));
            }
            if (globalState.staff) {
              localStorage.setItem('knsdc_staff', JSON.stringify(globalState.staff));
            }
          }
        } catch(e) {}

        await this.pushState(globalState);
        console.log('[LocalSync] Synced staff deletion across all roles and events to cloud state.');
      } catch (err) {
        console.warn('Failed to sync staff deletion to global state:', err);
      }

      // If the currently logged-in user in this session is the deleted staff, immediately lock out
      try {
        if (typeof localStorage !== 'undefined') {
          const curUserRaw = localStorage.getItem('kns_user');
          if (curUserRaw) {
            const u = JSON.parse(curUserRaw);
            if (u && u.email && u.email.trim().toLowerCase() === emailLower) {
              this.lockoutUser('Your staff account has been deleted by an Administrator.');
            }
          }
        }
      } catch(e) {}

      console.log('[LocalSync] Staff member deleted and locked out:', emailLower);
      return { success: true };
    } catch (err) {
      return { success: false, error: err.message };
    }
  ...
  // ── Validate if a user is an active staff member in staff_credentials ──
  async validateStaffSession(email, expectedRole = null) {
    if (!email) return { valid: false, reason: 'No session email found.' };
    const emailLower = email.trim().toLowerCase();
    if (emailLower === 'x@' || emailLower === 'x') {
      return { valid: false, reason: 'Account does not exist or has been deleted.' };
    }
    ...
    if (this.supabase) {
      try {
        ...
        if (!error && staffData) {
          return { valid: true, email: emailLower, role: staffData.role, name: staffData.name };
        }

        // Secondary check for judge_credentials
        const { data: judgeData } = await this.supabase
          .from('judge_credentials')
        ...
        if (judgeData) {
          return { valid: true, email: emailLower, role: 'judge', name: judgeData.name };
        }

        return { valid: false, reason: 'Staff account was deleted or revoked by Administrator.' };
      } catch (e) {
        console.warn('[LocalSync] validateStaffSession network notice:', e.message);
      }
    }

    // Local fallback check if network completely down
    const localStaff = (this.state && this.state.staff) || [];
    const found = localStaff.find(s => (s.email || '').trim().toLowerCase() === emailLower);
    if (found) {
      return { valid: true, email: emailLower, role: found.role, name: found.name };
    }

    return { valid: false, reason: 'Staff member not recognized in staff directory.' };
  }

  // ── Lockout User Overlay & Session Eviction ──
  lockoutUser(reason = 'Your staff account has been deleted by an Administrator.') {
    console.warn('[LocalSync] ⛔ LOCKOUT TRIGGERED:', reason);
    if (typeof localStorage !== 'undefined') {
    ...
    if (this.supabase && this.supabase.auth) {
      try { this.supabase.auth.signOut().catch(() => {}); } catch(e) {}
    }

    if (typeof document === 'undefined') return;
    if (document.getElementById('knsdc-lockout-overlay')) return;

    const overlay = document.createElement('div');
    overlay.id = 'knsdc-lockout-overlay';
    overlay.style.cssText = `
      position: fixed; inset: 0; z-index: 99999999;
      background: radial-gradient(circle at 50% 40%, rgba(26, 6, 12, 0.98), #090204 90%);
      display: flex; flex-direction: column; align-items: center; justify-content: center;
      color: #fff; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
      padding: 24px; text-align: center; backdrop-filter: blur(25px);
    `;
    overlay.innerHTML = `
      <div style="background: rgba(239, 68, 68, 0.12); border: 2px solid #ef4444; border-radius: 20px; padding: 40px 32px; max-width: 480px; width: 90%; box-shadow: 0 25px 60px rgba(239, 68, 68, 0.35); animation: knsdcLockPulse 1.5s infinite alternate;">
        <div style="font-size: 56px; margin-bottom: 16px;">⛔</div>
        <h2 style="font-size: 24px; font-weight: 800; color: #ef4444; margin: 0 0 12px 0; letter-spacing: -0.5px;">ACCOUNT LOCKED OUT</h2>
        <p style="font-size: 15px; color: #cbd5e1; line-height: 1.6; margin: 0 0 24px 0;">
          ${reason}
        </p>
        <div style="background: rgba(0,0,0,0.4); padding: 12px 18px; border-radius: 12px; font-size: 13px; color: #94a3b8; margin-bottom: 24px;">
          Only staff listed in the Admin Staff section may log in.<br>
          Redirecting to login page in <span id="lockout-countdown" style="color:#ef4444; font-weight:700;">3</span>s...
        </div>
        <button onclick="window.location.href='index.html'" style="background: #ef4444; color: #fff; border: none; padding: 12px 28px; border-radius: 10px; font-weight: 700; font-size: 14px; cursor: pointer; transition: 0.2s;">
          Return to Login
        </button>
      </div>
      <style>
        @keyframes knsdcLockPulse {
          0% { box-shadow: 0 20px 50px rgba(239, 68, 68, 0.2); }
          100% { box-shadow: 0 25px 70px rgba(239, 68, 68, 0.45); }
        }
      </style>
    `;
  ...
  // ── Enforce Staff Guard on Page Boot ──
  async enforceStaffGuard(requiredRoles = []) {
    if (typeof window === 'undefined') return true;
    const role = localStorage.getItem('kns_role');
    const userRaw = localStorage.getItem('kns_user');

    if (!role || !userRaw) {
      this.lockoutUser('You must log in to access this portal.');
      return false;
    }
    ...
    try {
      ...
      this.lockoutUser('Invalid session data. Please log in again.');
      return false;
    }

    if (!user || !user.email) {
      this.lockoutUser('Session email is missing. Please log in again.');
      return false;
    }

    if (requiredRoles.length > 0 && !requiredRoles.includes(role) && role !== 'admin') {
      this.lockoutUser(`Your role (${role}) is not authorized to access this portal.`);
      return false;
    }

    const validation = await this.validateStaffSession(user.email, role);
    if (!validation.valid) {
      this.lockoutUser(validation.reason || 'Your staff account has been deleted by an Administrator.');
      return false;
    }

    console.log(`[AuthGuard] Verified active staff member: ${user.email} (${validation.role})`);
    return true;
  }

  // ── Staff Authentication Heartbeat (Checks every 8 hours) ──
  startStaffHeartbeat() {
    if (this._staffHeartbeatStarted) return;
    this._staffHeartbeatStarted = true;

    const checkStaffStatus = async () => {
      if (typeof window === 'undefined' || typeof localStorage === 'undefined') return;
      const role = localStorage.getItem('kns_role');
      const userRaw = localStorage.getItem('kns_user');
      if (!role || !userRaw || role === 'public' || role === 'member') return;

      try {
        const u = JSON.parse(userRaw);
        if (u && u.email) {
          const res = await this.validateStaffSession(u.email, role);
          if (!res.valid) {
            console.warn('[LocalSync] Active staff session invalidated:', u.email, res.reason);
            this.lockoutUser(res.reason || 'Your staff account has been deleted by an Administrator.');
          }
        }
      } catch(e) {}
    };

    // Run periodic check every 8 hours (8 * 60 * 60 * 1000 ms)
    const EIGHT_HOURS_MS = 8 * 60 * 60 * 1000;
    setInterval(checkStaffStatus, EIGHT_HOURS_MS);
  }

  // ── Unified Staff & Member Sign In ──
  async signIn(email, password) {
    const emailNorm = email.trim().toLowerCase();

    // Reject dummy/deleted accounts immediately
    if (emailNorm === 'x@' || emailNorm === 'x') {
      return { success: false, error: 'Account does not exist or has been deleted.' };
    }

    // ── Step 1: Native Supabase Authentication ──
    if (this.supabase) {
      try {
        ...
        if (!error && data.user) {
          ...
          const isStaffRole = ['admin', 'monitor', 'host', 'director', 'judge', 'finance', 'sportsmanager', 'umpire', 'referee', 'service_partner'].includes(role);
          if (isStaffRole) {
            const val = await this.validateStaffSession(emailNorm, role);
            if (!val.valid) {
              await this.supabase.auth.signOut().catch(() => {});
              return { success: false, error: 'Access Denied: Your staff account was deleted or revoked by Administrator.' };
            }
          }
    ...
    // ── Step 1.5: Custom Staff Credentials Table (Admin Staff Section - Single Source of Truth) ──
    if (this.supabase) {
      try {
        ...
        if (staffData && !staffErr) {
          if (staffData.active === false) {
            return { success: false, error: 'Staff account has been deactivated by Administrator.' };
          }
          const role = staffData.role || 'monitor';
          localStorage.setItem('kns_role', role);
          localStorage.setItem('kns_user', JSON.stringify({ name: staffData.name, email: emailNorm }));
          console.log('[LocalSync] Staff authenticated via Admin Staff Section:', emailNorm, role);
          return {
            success: true,
            role: role,
    ...
    // ── Step 2: Judge Credentials Verification ──
    if (this.supabase) {
      try {
        const pHash = await sha256(password);
        const { data: judgeData, error: judgeErr } = await this.supabase
          .from('judge_credentials')
          .select('*')
          .eq('email', emailNorm)
          .eq('password_hash', pHash)
        ...
        if (judgeData && !judgeErr) {
          localStorage.setItem('kns_role', 'judge');
          localStorage.setItem('kns_user', JSON.stringify({ name: judgeData.name, email: emailNorm }));
          console.log('[LocalSync] Judge authenticated via judge_credentials:', emailNorm);
          return {
            success: true,
            role: 'judge',
            user: { email: emailNorm, user_metadata: { fullname: judgeData.name, role: 'judge' } }
          };
        }
      } catch(e) {}
    }

    return { success: false, error: 'Invalid credentials or unauthorized staff account.' };
  }
  ...
  // ─── Cloudflare R2 (Exclusive Zero-Egress Storage) File Upload & Delete ───────────────────────────
  async uploadFile(file, pathPrefix = '', customKey = '') {
    if (!file) return null;

    const prefix = pathPrefix ? pathPrefix.replace(/^\/+|\/+$/g, '') : 'participants';

    // 1. Exclusive: Upload directly to Cloudflare R2 (Zero Egress, Free Unlimited Bandwidth)
    try {
      const formData = new FormData();
      formData.append('file', file);
      formData.append('pathPrefix', prefix);
      if (customKey && typeof customKey === 'string' && customKey.trim()) {
        formData.append('customKey', customKey.trim());
      }
      ...
      console.warn('[LocalSync] Cloudflare R2 upload error:', r2Err);
    }

    // 2. Offline / Compression Fallback: Reads and compresses image file as compact Base64 Data URL
    // (Supabase Storage is strictly bypassed per configuration)
  ...
  async deleteFile(fileUrl) {
    ...
    // 1. If Cloudflare R2 file URL: Send delete command to Cloudflare Worker
    const isCloudflare = fileUrl.includes('r2.dev') || fileUrl.includes('workers.dev') || fileUrl.includes('sb2.kalikapurnabinsanghaclub') || !fileUrl.includes('supabase.co');
    if (isCloudflare) {
      const r2WorkerUrl = window.CLOUDFLARE_UPLOAD_URL || 'https://sb2.kalikapurnabinsanghaclub.workers.dev';
      let key = '';
      try {
      ...
      if (key) {
        try {
          const res = await fetch(`${r2WorkerUrl}?key=${encodeURIComponent(key)}`, {
            method: 'DELETE'
          });
          if (res.ok) {
            console.log('[LocalSync] Deleted file from Cloudflare R2:', key);
            return true;
          }
        } catch (delErr) {
          console.warn('[LocalSync] Cloudflare DELETE failed, trying POST delete:', delErr.message);
        }

        // Fallback to POST action=delete in case DELETE method is intercepted
        try {
          ...
          if (res.ok) {
            console.log('[LocalSync] Deleted file from Cloudflare R2 via POST action:', key);
            return true;
          }
        } catch (postErr) {
          console.error('[LocalSync] Failed to delete file from Cloudflare R2 via POST:', postErr.message);
        }
      }
    }

    // 2. Legacy fallback for old Supabase storage files if any still exist
    if (this.supabase && this.supabase.storage && fileUrl.includes('/public/knsdc-registration/')) {
      try {
        ...
        if (parts.length >= 2) {
          ...
          console.log('[LocalSync] Deleted legacy file from Supabase Storage:', filePath);
          return true;
        }
      } catch (err) {
        console.warn('[LocalSync] Supabase legacy delete notice:', err.message);
      }
    }
  ...
  async loadEvents() {
    try {
      ...
      const mappedEvents = data.map(dbEv => {
        ...
        let certSetup = dbEv.certificate_setup || dbEv.certificateSetup || (switchStates && switchStates.certificateSetup) || null;
        if (typeof certSetup === 'string') {
          try { certSetup = JSON.parse(certSetup); } catch(e) { certSetup = null; }
        }
        if (certSetup) {
          switchStates.certificateSetup = certSetup;
          if (!this.state.eventSwitches) this.state.eventSwitches = {};
          this.state.eventSwitches[dbEv.id] = { ...switchStates, certificateSetup: certSetup };
          this.state.eventSwitches[String(dbEv.id)] = { ...switchStates, certificateSetup: certSetup };
          if (dbEv.name || dbEv.title) {
            this.state.eventSwitches[dbEv.name || dbEv.title] = { ...switchStates, certificateSetup: certSetup };
          }
        }
        ...
        return {
          ...
          formFields,
          switchStates,
          certificateSetup: certSetup,
          certificate_setup: certSetup,
          ...
          venues: (() => {
            let vens = (eventVens && eventVens.length > 0) ? eventVens : (Array.isArray(directVens) && directVens.length > 0 ? directVens : []);
            if (vens.length === 0 && (dbEv.venue || dbEv.location)) {
              const vName = dbEv.venue || dbEv.location;
              vens = [{
                id: 1788864913467,
                name: vName,
                location: vName,
                eventId: dbEv.id,
                capacity: Number(dbEv.capacity) || 150,
                dates: dbEv.start_date || dbEv.date ? [dbEv.start_date || dbEv.date] : []
              }];
            }
            return vens;
          })()
  ...
  async updateEvent(id, eventObj) {
    try {
      ...
      if (eventObj.certificateSetup !== undefined || (eventObj.switchStates && eventObj.switchStates.certificateSetup !== undefined)) {
        const cs = eventObj.certificateSetup || (eventObj.switchStates && eventObj.switchStates.certificateSetup);
        if (cs) {
          const csStr = typeof cs === 'string' ? cs : JSON.stringify(cs);
          await tryCol({ certificate_setup: csStr });
          await tryCol({ certificateSetup: csStr });
        }
      }
      ...
      this.setData(state => {
        ...
        const mappedUpdate = {
          ...
          certificateSetup: eventObj.certificateSetup || (eventObj.switchStates && eventObj.switchStates.certificateSetup),
          certificate_setup: eventObj.certificateSetup || (eventObj.switchStates && eventObj.switchStates.certificateSetup),
  ...
  async stageDrop(participantId = null, venueId = null) {
    ...
    this.setData(state => {
      ...
      if (nextVenueStage.global && (!pidStr || String(nextVenueStage.global.currentOnStage) === pidStr)) {
        nextVenueStage.global = {
          currentOnStage: null,
          stageStatus: 'empty',
          transitionAt: now
        };
      }
      Object.keys(nextVenueStage).forEach(k => {
        if (nextVenueStage[k] && (!pidStr || String(nextVenueStage[k].currentOnStage) === pidStr)) {
          nextVenueStage[k] = { ...nextVenueStage[k], currentOnStage: null, stageStatus: 'empty', transitionAt: now };
        }
      });
  ...
  async permanentlyDeleteParticipant(id) {
    try {
      ...
      // Delete uploaded files from storage (Cloudflare R2 and legacy Supabase)
      if (p) {
        const filesToDelete = new Set();
        const checkUrl = (val) => {
          if (typeof val === 'string' && (val.startsWith('http://') || val.startsWith('https://'))) {
            filesToDelete.add(val);
          }
        };
        if (p.musicUrl) checkUrl(p.musicUrl);
        if (p.photo) checkUrl(p.photo);
        if (p.formAnswers) {
        ...
        for (const fileUrl of filesToDelete) {
          try {
            console.log('[LocalSync] Permanently deleting file:', fileUrl);
            await this.deleteFile(fileUrl);
          } catch (delErr) {
            console.warn('[LocalSync] Error permanently deleting file:', fileUrl, delErr);
          }
        }
      }
    ...
    try {
      ...
      const mappedParts = data.map(p => {
        ...
        return {
          ...
          venueId: p.venue ? (isNaN(Number(p.venue)) ? p.venue : (Number(p.venue) || p.venue)) : ((formAns && (formAns['Venue Field'] || formAns['Venue'] || formAns['venue'])) || existing.venueId || null),
          venue: p.venue || (formAns && (formAns['Venue Field'] || formAns['Venue'] || formAns['venue'])) || existing.venue || '',
