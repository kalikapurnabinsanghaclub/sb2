...
class LocalSync {
  ...
  async updateEvent(id, eventObj) {
    try {
      // ── STEP 1: Build the core update with ONLY columns guaranteed to exist ──
      // These columns are always present per the original schema.
      const coreObj = {};
      if (eventObj.name !== undefined)        coreObj.title       = eventObj.name;
      if (eventObj.org !== undefined)         coreObj.organizer   = eventObj.org;
      if (eventObj.type !== undefined)        coreObj.category    = eventObj.type;
      if (eventObj.venue !== undefined)       coreObj.venue       = eventObj.venue;
      if (eventObj.startDate !== undefined)   coreObj.date        = eventObj.startDate;
      if (eventObj.startTime !== undefined)   coreObj.time        = eventObj.startTime;
      if (eventObj.banner !== undefined)      coreObj.image       = eventObj.banner;
      if (eventObj.description !== undefined) coreObj.description = eventObj.description;
      if (eventObj.active !== undefined)      coreObj.active      = eventObj.active;
      if (eventObj.formFields !== undefined) {
        coreObj.form_fields = typeof eventObj.formFields === 'string'
          ? eventObj.formFields
      ...
      if (coreError) {
        ...
        // Return error message so admin toast can show details
        return { success: false, message: coreError.message || coreError.details || 'Database error on core fields' };
      }

      // ── STEP 2: Try extended columns (may not exist yet — fail silently) ──
      const extObj = {};
      if (eventObj.endDate !== undefined)       extObj.end_date       = eventObj.endDate || null;
      if (eventObj.endTime !== undefined)       extObj.end_time       = eventObj.endTime || null;
      if (eventObj.capacity !== undefined)      extObj.capacity       = eventObj.capacity || null;
      if (eventObj.switchStates !== undefined)  extObj.switch_states  = eventObj.switchStates;
      if (eventObj.roundSchedules !== undefined) {
        extObj.round_schedules = typeof eventObj.roundSchedules === 'string'
          ? eventObj.roundSchedules
          : JSON.stringify(eventObj.roundSchedules);
      }

      if (Object.keys(extObj).length > 0) {
        const { error: extError } = await this.supabase.from('events').update(extObj).eq('id', id);
        if (extError) {
          // Extended columns may not exist yet — log but don't fail the whole operation
          console.warn('[LocalSync] Extended columns update skipped (columns may not exist yet):', extError.message);
        }
      }
