...
class LocalSync {
    ...
    try {
      ...
      if (data && data.payload) {
        ...
        if (!this.state.lastUpdated || fetchedState.lastUpdated > this.state.lastUpdated) {
          // Preserve switch data we just merged
          ...
          // Re-apply preserved switch data (localStorage + cloud merged)
  ...
  setData(updater) {
    ...
    // Persist critical switch states to localStorage for instant reload recovery
    try {
  ...
  async loadEvents() {
    try {
      ...
      // IMPORTANT: Preserve existing switch data when loading events
      // loadEvents should ONLY update events, not overwrite switch states
      ...
      this.setData(state => ({
        ...
        // Re-apply preserved switch data to prevent race condition overwrites
