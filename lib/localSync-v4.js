...
let supabaseUrl = window.supabaseUrl || "https://fjscpohgysbelzkrkrxm.supabase.co";
let supabaseAnonKey = window.supabaseAnonKey || "sb_publishable_veI5vYBOXffm4FSPjobycA_95FiB6a5";
...
class LocalSync {
  ...
  async loadEvents() {
    try {
      const { data, error } = await this.supabase.from('events').select('id, name, org, type, venue, start_date, start_time, end_date, end_time, capacity, banner, description, staff, round_schedules, active, form_fields, switch_states, category, organizer, publicReg, stagePreview, resultPublic, title, date, time, image, whatsapp, facebook, whatsapp_number, facebook_url, created_at, updated_at');
      if (error) {
      ...
      try {
        const { data: vData } = await this.supabase.from('venues').select('id, name, location, event_id, created_at');
      ...
      try {
        const { data: aData } = await this.supabase.from('judge_agreements').select('id, name, phone, email, password, city, event_id, date, date_upto, time, venue_id, venue_name, spec, amount, advance, notes, status, submitted, payment_received, photo_url, agreed_tc, signature').order('created_at', { ascending: false });
        if (aData) {
          agrData = aData.map(a => ({
            id: Number(a.id) || a.id,
            name: a.name,
    ...
    try {
      ...
      const mappedParts = data.map(p => {
        ...
        return {
          ...
          formAnswers: formAns,
          present: p.present !== undefined ? p.present : (existing.present !== undefined ? existing.present : false),
          ...
          round: p.round || existing.round || 'audition',
          stageStatus: p.stage_status || existing.stageStatus || 'waiting',
