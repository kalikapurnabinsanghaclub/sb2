...
class LocalSync {
  ...
  async fetchSupabaseState() {
    ...
    try {
      ...
      if (data && data.payload) {
        ...
        // Preserve switch data and assignments we just merged
        ...
        // Re-apply preserved switch data (localStorage + cloud merged) and assignments
  ...
  async deleteParticipant(id) {
    try {
      ...
      // 1. Auto-Wipe Files from Storage
      if (p && p.formAnswers) {
        const filesToDelete = [];
        const checkValueForUrl = (val) => {
        ...
        Object.values(p.formAnswers).forEach(val => {
          if (Array.isArray(val)) val.forEach(v => checkValueForUrl(v));
          else checkValueForUrl(val);
        });
        
        if (filesToDelete.length > 0) {
          console.log('[LocalSync] Auto-deleting files:', filesToDelete);
